...
TEST_SUITE(TESTQ(SHA256Tests)) TEST_SUITE_OPEN
	...
	TEST_CASE(TESTQ(testSHA256)) {
		...
		CHECK_MESSAGE(test == "0CB43D2D948242706CFC50A337861141BE5A4FD9ED2EAEB3DB0CBD9A771260C8", TEST_MESSAGE(Object hashing failed));
			//Test extracting intermediate hashes
		SHA256 intermittent;
		intermittent << firstValue;
		auto unused = intermittent.hexHash();
		intermittent << secondValue;
		CHECK_MESSAGE(intermittent.hexHash() == (SHA256() << (firstValue + secondValue)).hexHash(), TEST_MESSAGE(Intermittent hashing failed));
	}
