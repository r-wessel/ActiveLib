...
namespace active {
	...
	public:
		 ...
		 */
		BufferIn(const file::File& sourceFile, std::optional<text_format> format = text_format{}) {	setSource(sourceFile, format); }
		/*!
		 Constructor
		 ...
		 @param format The source data format (nullopt = attempt to discover format from source)
		 */
		BufferIn(const Memory& sourceMem, std::optional<text_format> format = text_format{}) { setSource(sourceMem, format); }
		/*!
		 Constructor
		 ...
		 @param format The source data format (nullopt = attempt to discover format from source)
		 */
		BufferIn(const string& sourceString, std::optional<text_format> format = text_format{}) { setSource(sourceString, format); }
		/*!
		 Move constructor
		 ...
		 */
		Memory::size_type getPosition() const { return (m_file == nullptr) ? m_readPos : getFilePosition(*m_file); }
		/*!
		...
		const BufferIn& get(string& dest) const {
			string incoming;
			if (getEncodedChar(incoming) > 0) [[likely]]
	...
	private:
		...
		bool checkEndOfFile() const {
			if (eof()) [[unlikely]]
				return true;
			if ((m_remaining > 0) || (m_readPos < m_bufferLen)) [[likely]]
				return false;
		...
		/*!
		 ...
		 @param file The file to get the position from
		 @return The file read position
		 */
		Memory::size_type getFilePosition(const file::File& file) const;
		/*!
