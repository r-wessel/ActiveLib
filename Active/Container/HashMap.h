...
#ifndef ACTIVE_CONTAINER_HASH_MAP
#define ACTIVE_CONTAINER_HASH_MAP
...
namespace active::container {
	...
	*/
	...
	public:
		...
		*/
		HashMap(const HashMap& source) : base() { cloneFrom(source); }
		/*!
			Move constructor
			@param source The object to move
		*/
		HashMap(HashMap&& source) : base(std::move(source)) {}
		/*!
			Destructor
		*/
		...
		*/
		auto insert(const raw_type& item) { return emplace(item.first, std::move(clone(item.second))); }
		/*!
		...
		*/
		auto emplace(const Key& key, mapped_type&& item) { return base::emplace(std::make_pair(key, std::move(item))); }
		/*!
			Emplace a key/value pair into the map
			@param item The item to push
		*/
	...
	private:
		/*!
			...
			@param source The object to copy
		*/
		void cloneFrom(const HashMap& source) {
			for (const auto& item : source)
				base::emplace(item.first, std::unique_ptr<T>{item.second ? clone(*item.second) : nullptr});
		}
	};
	
}

#endif	//ACTIVE_CONTAINER_HASH_MAP
