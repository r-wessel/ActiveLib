...
#ifndef ACTIVE_CONTAINER_MULTI_MAP
#define ACTIVE_CONTAINER_MULTI_MAP
...
namespace active::container {
	...
	/*!
		...
		This allows multi-multimap to store objects with polymorphic behaviour and object copying (object type must conform to Clonable).
		Keep in mind that the normal behaviour of the multi-multimap is to take ownership of any objects stored in it (unless release is used)
	...
	*/
	...
	class MultiMap : public std::multimap<Key, std::unique_ptr<T>> {
	public:
		...
		*/
		MultiMap(const MultiMap& source) : base () { cloneFrom(source); }
		/*!
			Move constructor
			@param source The object to move
		*/
		MultiMap(MultiMap&& source) : base(std::move(source)) {}
		/*!
			Destructor
		*/
	...
	private:
		/*!
			Clone the contents of another multimap into this
			@param source The object to copy
		*/
		void cloneFrom(const MultiMap& source) {
			for (const auto& item : source)
				base::insert(std::make_pair(item.first, item.second ? clone(*item.second) : mapped_type()));
		}
	};
	
}

#endif	//ACTIVE_CONTAINER_MULTI_MAP
